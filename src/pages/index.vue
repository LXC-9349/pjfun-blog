<template>
  <div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800">
    ...
    <header class="py-4 px-4 sm:px-6 lg:px-8 border-b border-gray-200 dark:border-gray-700 bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm">
      <div class="max-w-7xl mx-auto flex justify-between items-center">
        ...
        <div class="flex items-center space-x-4">
          ...
          <button @click="toggleLanguage" class="hidden sm:flex items-center gap-2 px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 text-sm hover:bg-gray-50 dark:hover:bg-gray-700">
            <IconCarbonLanguage class="w-4 h-4" />
            {{ currentLang === 'zh' ? 'EN' : '中文' }}
          </button>
          <ThemeToggle />
          <button @click="toggleMobileMenu" class="lg:hidden p-2 rounded-lg border border-gray-300 dark:border-gray-600">
    ...
    <div class="md:hidden px-4 pt-4">
      <div class="relative">
        <input type="text" :placeholder="t('searchPlaceholder')" class="w-full py-3.5 px-5 rounded-xl shadow-sm border" @click="openSearch" readonly>
        <IconCarbonSearch class="absolute right-4 top-1/2 -translate-y-1/2 w-6 h-6 text-gray-400" />
      </div>
    </div>
        ...
        <div class="flex items-center gap-3 flex-wrap">
          ...
          <button
              ...
              selectedTag === tag
                ...
                : 'bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 hover:border-blue-400'
            ]"
          >
            {{ tag }}
          </button>
      ...
      <div class="flex flex-col lg:flex-row gap-8">
        ...
        <main class="flex-1 min-w-0">
          ...
          <div v-else class="grid gap-8 sm:grid-cols-1 xl:grid-cols-1 auto-rows-fr">
                ...
                <div class="p-6 flex flex-col flex-1">
                  ...
                  <div class="flex flex-wrap gap-2 mt-4">
                    <span
                        ...
                        :key="tag"
                        class="px-3 py-1 text-xs rounded-full bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300"
                    >
                      {{ tag }}
                    </span>
                  </div>
                  ...
                  <div class="flex justify-between items-center mt-6 text-xs text-gray-500 dark:text-gray-400">
                    ...
                    <span class="flex items-center font-medium text-blue-600 dark:text-blue-400 group-hover:underline">
                      {{ t('readMore') }}
                      <IconCarbonArrowRight class="w-4 h-4 ml-1 group-hover:translate-x-1 transition-transform" />
                    </span>
                  </div>
                </div>
              </router-link>
            ...
            <!-- 加载更多按钮（无限滚动触发器） -->
            <div v-if="hasMore && !loading" ref="loadMoreTrigger" class="col-span-full text-center py-12">
        ...
        <aside class="hidden xl:block w-80 flex-shrink-0">
          <div class="sticky top-6 bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6">
            ...
            <div class="space-y-4">
              ...
              >
                <div class="w-10 h-10 rounded-full bg-gradient-to-r from-blue-500 to-purple-600 flex items-center justify-center text-white text-sm font-bold flex-shrink-0">
                  {{ i + 1 }}
                </div>
                <div class="flex-1 min-w-0">
...
import { HOT_TAGS } from '@/constants'
...
const hasMore = computed(() => {
  ...
  return displayedPosts.value.length < total
})
...
onMounted(async () => {
  try {
    const [navRes, treeRes] = await Promise.all([
      fetch('/generated/nav.json'),
      fetch('/generated/tree.json')
    ])
